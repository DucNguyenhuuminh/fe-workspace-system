...
const ShareModal = ({ isOpen, onClose, fileId, fileName }: ShareModalProps) => {
  ...
  const handleRevoke = async (token: string) => {
    ...
    try {
      ...
      toast.error("Lỗi khi thu hồi liên kết");
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] bg-white">
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-4">
          <h3 className="font-semibold text-sm text-slate-800">Tạo liên kết mới</h3>
          
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-500 flex items-center gap-1"><Clock className="h-3 w-3"/> Hết hạn sau</label>
              <Select value={expiresIn} onValueChange={setExpiresIn}>
                <SelectTrigger className="bg-white"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="0">Không bao giờ</SelectItem>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-500 flex items-center gap-1"><Lock className="h-3 w-3"/> Mật khẩu (Tùy chọn)</label>
              <Input type="text" placeholder="Để trống nếu public..." value={password} onChange={(e) => setPassword(e.target.value)} className="bg-white" />
            </div>
          </div>
        ...
        <div className="mt-2 space-y-3">
          <h3 className="font-semibold text-sm text-slate-800">Các liên kết đang hoạt động</h3>
          <div className="max-h-[250px] overflow-y-auto space-y-2 pr-1">
            {isLoading ? (
              <div className="flex justify-center py-4"><Loader2 className="h-5 w-5 animate-spin text-slate-400" /></div>
            ) : links.length === 0 ? (
              <p className="text-center text-sm text-slate-500 py-4">Chưa có liên kết nào được tạo.</p>
            ) : (
              links.map((link) => (
                <div key={link.token} className={`p-3 rounded-lg border flex items-center justify-between ${link.isRevoked ? 'bg-red-50 border-red-100 opacity-60' : 'bg-white border-slate-200'}`}>
                  <div className="flex-1 min-w-0 pr-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {link.password && <Lock className="h-3 w-3 text-amber-500" aria-label="Có mật khẩu" />}
                      {!link.settings?.allowedDownload && <Shield className="h-3 w-3 text-red-400" aria-label="Chặn tải xuống" />}
                    </div>
                    <p className="text-xs text-slate-500">
                      Tạo lúc: {formatDate(link.createdAt)} 
                      {link.expiredAt && ` • Hết hạn: ${formatDate(link.expiredAt)}`}
                      {link.isRevoked && <span className="text-red-500 font-medium ml-2">(Đã thu hồi)</span>}
                    </p>
                  </div>
