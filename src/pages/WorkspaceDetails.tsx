...
const WorkspaceDetail = () => {
  ...
  const [destPath, setDestPath] = useState<{id: string | null, name: string}[]>([{id: null, name: 'Loading...'}]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-0.5">
                <span className="font-medium uppercase tracking-tight">Permission: {memberInfo?.permissions || "viewer"}</span>
                {isAdmin && <span title="You are Admin"><ShieldCheck className="h-4 w-4 text-primary" /></span>}
              </div>
            </div>
          </div>
            ...
            {isAdmin && !currentFolderInfo && (
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="outline" className="gap-2"><Settings className="h-4 w-4" /> Members</Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[550px] bg-[#f4f6fc] p-6 border-none shadow-2xl">
                  <DialogHeader className="mb-4">
                    <DialogTitle className="text-xl font-semibold text-slate-800">Manage workspace members</DialogTitle>
                  </DialogHeader>
                  <div className="flex gap-3 items-center">
                    <div className="flex-1 relative">
                      <Input placeholder="Enter email..." value={newMemberEmail} onChange={(e) => setNewMemberEmail(e.target.value)} className="w-full h-12 bg-transparent border-2 border-indigo-500 rounded-xl focus-visible:ring-0 focus-visible:border-indigo-600 shadow-sm text-slate-700"/>
                    </div>
                    ...
                    {currentWorkspace?.members.map((m) => (
                      <div key={m.userId.toString()} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors">
                        <div className="flex-1 min-w-0 mr-4">
                          ...
                          <p className="text-[13px] text-slate-500 mt-1">Role: {m.role} &bull; Permission: {m.permissions}</p>
                        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <div className="flex items-center justify-between mb-6">
            <TabsList className="bg-secondary/50">
              ...
              <TabsTrigger value="posts" className="px-6">Discussions</TabsTrigger>
            </TabsList>
            ...
            {activeTab === "files" && (
              <div className="flex items-center gap-3">
                {isEditor && (
                  <Button onClick={openCreateFolderModal} className="gap-2 shadow-sm"><FolderPlus className="h-4 w-4" /> Create folder</Button>
                )}
                    ...
                    <DialogContent className="sm:max-w-[700px] bg-[#f4f6fc] p-6 border-none shadow-2xl">
                      <DialogHeader className="mb-4 flex flex-row items-center justify-between pr-6">
                        <DialogTitle className="text-xl font-semibold text-slate-800 flex items-center gap-2">
                          <Trash2 className="h-5 w-5 text-destructive" /> Workspace Trash
                        </DialogTitle>
                        {trashedItems.length > 0 && (
                          <Button variant="destructive" size="sm" className="gap-2" onClick={emptyAllTrash}>
                            <AlertOctagon className="h-4 w-4" /> Empty Trash
                          </Button>
                        )}
                      </DialogHeader>
                              ...
                              <div key={display.id} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors group">
                                <div className="flex items-center gap-4 min-w-0 pr-4 flex-1">
                                  ...
                                  <div className="overflow-hidden">
                                    ...
                                    <p className="text-xs text-slate-500 mt-0.5">Deleted on: {display.date}</p>
                                  </div>
                                </div>
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                  ...
                                  <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 px-2" title="Delete permanently" onClick={() => forceDeleteItem(display.id, item.kind)}>
          ...
          <TabsContent value="files" className="mt-0 outline-none">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-32 text-center border-2 border-dashed border-border/60 rounded-2xl bg-secondary/20 transition-all hover:bg-secondary/30">
                ...
                <h3 className="text-xl font-semibold text-foreground">This workspace is empty</h3>
                <p className="text-muted-foreground mt-2 max-w-sm">{isEditor ? "Start uploading documents or create a new folder to share with the team." : "This team has no documents yet."}</p>
                {isEditor && <div className="mt-6 flex gap-3"><Button onClick={openCreateFolderModal} className="gap-2 shadow-sm"><FolderPlus className="h-4 w-4" /> Create folder</Button></div>}
              </div>
            ) : viewMode === "list" ? (
                      ...
                      <div key={display.id} className="grid grid-cols-12 px-6 py-4 items-center hover:bg-secondary/60 transition-all cursor-pointer group" onDoubleClick={() => handleDoubleClick(item)}>
                        ...
                        <div className="col-span-1 text-right">
                          <FileContextMenu 
                            ...
                            onRename={() => openRenameModal(item)} onDelete={() => handleDelete(item)} onDownload={() => downloadDocument(display.id, display.name)}
        ...
        <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden bg-background border-border">
          <DialogHeader className="p-6 pb-4 border-b">
            <DialogTitle className="text-lg font-semibold flex items-center gap-2">
              <FolderOutput className="h-5 w-5 text-indigo-500" /> Move to...
            </DialogTitle>
          </DialogHeader>
